/*
 * SPDX-FileCopyrightText: Copyright 2026 Free Software Foundation, Inc.
 * SPDX-FileCopyrightText: 2026 GNU Radio contributors
 *
 * This file is part of GNU Radio
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 */

/***********************************************************************************/
/* This file is automatically generated using bindtool and can be manually edited  */
/* The following lines can be configured to regenerate this file during cmake      */
/* If manual edits are made, the following tags should be modified accordingly.    */
/* BINDTOOL_GEN_AUTOMATIC(0)                                                       */
/* BINDTOOL_USE_PYGCCXML(0)                                                        */
/* BINDTOOL_HEADER_FILE(rssi_tag_cc.h)                                        */
/* BINDTOOL_HEADER_FILE_HASH(df0d682f5085b903c92c64fc44555618)                     */
/***********************************************************************************/

#include <pybind11/complex.h>
#include <pybind11/pybind11.h>
#include <pybind11/stl.h>

namespace py = pybind11;

#include <gnuradio/wisun/rssi_tag_cc.h>
// pydoc.h is automatically generated in the build directory
#include <rssi_tag_cc_pydoc.h>

void bind_rssi_tag_cc(py::module& m)
{

    using rssi_tag_cc = ::gr::wisun::rssi_tag_cc;


    py::class_<rssi_tag_cc,
               gr::sync_block,
               gr::block,
               gr::basic_block,
               std::shared_ptr<rssi_tag_cc>>(m, "rssi_tag_cc", D(rssi_tag_cc))

        .def(py::init(&rssi_tag_cc::make), py::arg("n_samples"), D(rssi_tag_cc, make))


        ;
}
